...
/**
 * Stringfunctions class contains various methods for Strings 1. to compare 2.
 * to reverse 3. to change case 4. to return largest word
 * 
 * @author Twinkle-Garg
 *
 */
public class StringFunctions {
    ...
    public String reverse(String inputString) {
        ...
        for (int counter = 0; counter < inputString.length(); counter++) {
            reversedString += inputString.charAt(inputString.length() - counter
                    - 1);
        }
    ...
    public String changeCase(String inputString) {
        ...
        for (int counter = 0; counter < length; counter++) {
            if (inputString.charAt(counter) >= 97
                    && inputString.charAt(counter) <= 122) {
                int requiredAscii = (int) (inputString.charAt(counter)) - 32;
                char required = (char) (requiredAscii);
                changedString += required;
            } else if (inputString.charAt(counter) >= 65
                    && inputString.charAt(counter) <= 90) {
                int requiredAscii = (int) (inputString.charAt(counter)) + 32;
                char required = (char) (requiredAscii);
                changedString += required;
            } else {
    ...
    /**
     * This method will take a string from user and compute word of maximum
     * length seperated by whitespace
     * 
     * @return word of maximum length
     */
    public String largestWord(String input1) {
        String maximum = "";
        String checkWord = "";
        int maxLength = 0;
        int length = 0;
